...
class StockDataProducer:
    ...
    def _fetch_incremental(self, ticker: str, last_timestamp: datetime) -> List[Dict]:
        """Fetch real-time incremental data using yfinance"""
        try:
            # Calculate gap between last_timestamp and now
            now = datetime.now(UTC_TZ)
            gap_days = (now - last_timestamp).total_seconds() / (24 * 3600)

            # If gap is too large (>2 days), we need to catch up
            if gap_days > 2.1:  # 2.1 to account for timezone differences
                logger.warning(f"Large gap detected for {ticker}: {gap_days:.1f} days since last update")
                # Use direct Yahoo API to catch up (more reliable for historical data)
                return self._fetch_direct_yahoo_finance_incremental_catchup(ticker, last_timestamp)

            # Normal case: small gap, use yfinance for real-time data
            stock = yf.Ticker(ticker)
            df = stock.history(period="2d", interval="1m", start=last_timestamp, prepost=False)

            if df.empty:
                logger.debug(f"No new real-time data for {ticker}")
                return []

            return self._process_dataframe(ticker, df, "incremental", "1m", None, None)

        except Exception as e:
            logger.error(f"Real-time fetch failed for {ticker}: {e}")
            return []
    ...
    def process_ticker(self, ticker: str):
        ...
        try:
            ...
            for message in messages:
                ...
                if success:
                    # Track the latest successfully published timestamp
            ...
            # Update state only with successfully published data
            if latest_successful_timestamp:
                self.state_manager.update_timestamp(ticker, latest_successful_timestamp)
